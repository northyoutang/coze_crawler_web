...
async def startup_event():
    ...
    # 从数据库加载Token到token_keeper
    from database import SessionLocal
    db = SessionLocal()
    try:
        setting = db.query(Setting).first()
        if setting and setting.ms_token:
            token_keeper.set_token(setting.ms_token)
            token_keeper.start()
            print(f"已从数据库加载 msToken: {setting.ms_token[:20]}...")
    finally:
        db.close()
...
async def get_token_interactive(db: Session = Depends(get_db), auth: bool = Depends(require_auth)):
    """启动浏览器交互式获取Token"""
...
async def set_token(token_data: dict, db: Session = Depends(get_db), auth: bool = Depends(require_auth)):
    """手动设置Token"""
